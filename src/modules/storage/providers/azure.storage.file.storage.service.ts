...
export class AzureStorageFileStorageService implements IFileStorageService {
    ...
    upload = async (inputStream: any, storageKey: string): Promise<string> => {
        try {
            ...
            await blockBlobClient.uploadStream(inputStream);

            Logger.instance().log(`Successfully uploaded ${storageKey} to Azure Storage`);
            return storageKey;
        }
        catch (error) {
    ...
    uploadLocally = async (storageKey: string, localFilePath: string): Promise<string> => {
        try {
            ...
            Logger.instance().log(`Successfully uploaded local file ${localFilePath} as ${storageKey} to Azure Storage`);
            return storageKey;
        }
        catch (error) {
            Logger.instance().log(error.message);
    ...
    rename = async (storageKey: string, newFileName: string): Promise<boolean> => {
        try {
            ...
            Logger.instance().log(`Successfully renamed ${storageKey} to ${newPath}`);
            return true;
        }
        catch (error) {
            Logger.instance().log(error.message);
