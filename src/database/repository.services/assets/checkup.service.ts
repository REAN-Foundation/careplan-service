import {
    CheckupModel
} from '../../models/assets/checkup.model';
import {
    UserModel
...
export class CheckupService {
    ...
    create = async (createModel: CheckupCreateModel) => {
        try {
            if (!createModel.AssetCode) {
                createModel.AssetCode = AssetHelper.generateAssetCode(AssetType.Checkup, createModel.Name);
            }
    ...
    exists = async (id): Promise < boolean > => {
        try {
            const record = await this.Checkup.findByPk(id);
            return record !== null;
        } catch (error) {
    ...
    search = async (filters: CheckupSearchFilters): Promise < CheckupSearchResults > => {
        try {
            ...
            const foundResults = await this.Checkup.findAndCountAll(search);
            const searchResults: CheckupSearchResults = {
    ...
    update = async (id, updateModel) => {
        try {
            if (Object.keys(updateModel).length > 0) {
                var res = await this.Checkup.update(updateModel, {
                    where : {
                        id : id
                    }
                });
                if (res.length !== 1) {
                    throw new Error('Unable to update checkup!');
                }
            }
    ...
    delete = async (id) => {
        try {
            var result = await this.Checkup.destroy({
                where : {
                    id : id
                }
            });
