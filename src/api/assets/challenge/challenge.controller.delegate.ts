...
import {
    ChallengeService
} from '../../../database/repository.services/assets/challenge.service';
import {
    ErrorHandler
...
import {
    ChallengeValidator as validator
...
export class ChallengeControllerDelegate {
    ...
    constructor() {
        this._service = new ChallengeService();
    }
    ...
    create = async (requestBody: any) => {
        ...
        var createModel: ChallengeCreateModel = this.getCreateModel(requestBody);
        var record = await this._service.create(createModel);
        if (record === null) {
            throw new ApiError('Unable to create challenge!', 400);
        }
    ...
    getById = async (id: uuid) => {
        ...
        if (record === null) {
            ErrorHandler.throwNotFoundError('Challenge with id ' + id.toString() + ' cannot be found!');
        }
        return this.getEnrichedDto(record);
    };
    ...
    search = async (request: express.Request) => {
        ...
        var filters: ChallengeSearchFilters = this.getSearchFilters(request.query);
        filters = await this.authorizeSearch(request, filters);
        var searchResults: ChallengeSearchResults = await this._service.search(filters);
    ...
    update = async (id: uuid, requestBody: any) => {
        ...
        const updateModel: ChallengeUpdateModel = this.getUpdateModel(requestBody);
        const updated = await this._service.update(id, updateModel);
        if (updated == null) {
            throw new ApiError('Unable to update challenge!', 400);
        }
    ...
    delete = async (id: uuid) => {
        ...
        const challengeDeleted: boolean = await this._service.delete(id);
        return {
    ...
    getUpdateModel = (requestBody): ChallengeUpdateModel => {

        const updateModel: ChallengeUpdateModel = {};
    ...
    getCreateModel = (requestBody): ChallengeCreateModel => {
        return {
